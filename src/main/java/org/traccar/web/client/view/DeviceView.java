...
public class DeviceView implements RowMouseDownEvent.RowMouseDownHandler, CellDoubleClickEvent.CellDoubleClickHandler {
    ...
    private static final EventProperties eventProperties = GWT.create(EventProperties.class);
    ...
    /**
     * Tab Event All Device From Device Event
    */ 
    
    @UiField(provided = true)
    TabItemConfig eventTabConfig;
        
//    @UiField(provided = true)
//    ColumnModel<DeviceEvent> cm;
//    
//    @UiField(provided = true)
//    ListStore<DeviceEvent> eventStore;
// 
//    @UiField
//    GridView<DeviceEvent> gridView;
    
//    @UiField
//    Grid<DeviceEvent> gridEvent;
    ...
    public DeviceView(final DeviceHandler deviceHandler,
         ...
         * Column Speed
         */
        ColumnConfig<Device, Double> colidleSpeedThreshold = new ColumnConfig<Device, Double>(deviceProperties.idleSpeedThreshold(), 60, i18n.speed());
        colidleSpeedThreshold.setFixed(true);
        colidleSpeedThreshold.setResizable(false);
        columnConfigList.add(colidleSpeedThreshold);
        ...
        settingsEvent.setVisible(!readOnly && (admin || manager));
    ...
    /**
     * Sub Menu Event digunakan untuk admin
     * Afes - Sub Menu Event
     */
    @UiHandler("settingsEvent")
    public void onShowEventLog(SelectionEvent<Item> event) {
    ...
    private ColumnModel<DeviceEvent> initColumnEvent() {
        ColumnConfig<DeviceEvent, Date> timeCol    = new ColumnConfig<DeviceEvent, Date>(eventProperties.time(), 50, "Time");
        ColumnConfig<DeviceEvent, String> nameCol  = new ColumnConfig<DeviceEvent, String>(eventProperties.name(), 80, "Name");
//        ColumnConfig<DeviceEvent, String> eventCol = new ColumnConfig<DeviceEvent, String>(eventProperties.type(), 120, "Type");
        
        List<ColumnConfig<DeviceEvent, ?>> columns = new ArrayList<ColumnConfig<DeviceEvent, ?>>();        
        columns.add(timeCol);
        columns.add(nameCol);
//        columns.add(eventCol);

        ColumnModel<DeviceEvent> columnModel = new ColumnModel<DeviceEvent>(columns);

        return columnModel;
    }
    
    private ListStore<DeviceEvent> initListStore() {
    ListStore<DeviceEvent> listStore = new ListStore<DeviceEvent>(eventProperties.id());
